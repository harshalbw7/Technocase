...
<html lang="en">
   ...
   <body class="main-layout inner_page" background="../home/images/p8.png">
        ...
        <div class="container">
           <div class="row">
              <div class="col-md-12">
                 ...
                 <h2>CASE STUDY RESONSE</h2><br><br> 
                 <?php
                       $conn = mysqli_connect('localhost', 'root', '', 'technocase');
                       $name = $_POST['partname'];
                       $sql = "SELECT participant.participantID,participant.name, participant.college
                               FROM participant
                               inner JOIN casestudyresponse ON participant.participantID = casestudyresponse.participantID
      ...
      <?php
         $conn = mysqli_connect('localhost', 'root', '', 'technocase');
         $sql = "Select question,fname from casestudyresponse where participantID=$id";
         $result = mysqli_query($conn,$sql);
         if(mysqli_num_rows($result)>0){
            if($row = mysqli_fetch_assoc($result)){
                $q=$row['question'];
    ...
    <form method="post" action="../php resource/J_caseresult.php">
      ...
      <button type="submit" class="submit-button" style="background-color : darkorange" name="submit"><b>Submit Marks</b></button>
      <?php $fname=(string)$f;?>
